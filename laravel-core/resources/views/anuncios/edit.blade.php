...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-black text-primary-dark">Editar anuncio</h1>
            <p class="text-gray-400 text-sm">{{ $anuncio->titulo ?? 'Sin título' }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <form method="POST" action="{{ route('anuncios.update', $anuncio) }}" enctype="multipart/form-data"
            ...
            @csrf
            @method('PUT')
            @include('anuncios._form', ['anuncio' => $anuncio])
            ...
            <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                <form method="POST" action="{{ route('anuncios.destroy', $anuncio) }}"
                    ...
                    @csrf @method('DELETE')
                    <button type="submit"
                            class="px-4 py-2 rounded-xl text-xs font-bold text-red-400
