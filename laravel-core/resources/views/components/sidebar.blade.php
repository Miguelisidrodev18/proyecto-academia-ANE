...
<aside class="fixed inset-y-0 left-0 z-30 flex flex-col transition-all duration-300 ease-in-out md:translate-x-0 overflow-hidden"
       :class="{ '-translate-x-full': !sidebarOpen }"
       :style="'background: linear-gradient(180deg, #071e3d 0%, #082B59 40%, #0a3266 100%); ' + (sidebarCollapsed ? 'width:64px' : 'width:256px')">

    {{-- ── Logo ─────────────────────────────────────────────────── --}}
    <div class="flex items-center h-16 border-b border-white/10 flex-shrink-0 overflow-hidden transition-all duration-300"
        ...
        <div class="overflow-hidden transition-all duration-300 whitespace-nowrap leading-tight min-w-0"
    ...
    <a href="{{ route('profile.edit') }}"
       class="mx-3 mt-3 mb-1 rounded-xl flex items-center group
              bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20
              transition-all duration-300 cursor-pointer flex-shrink-0 overflow-hidden"
       :class="sidebarCollapsed ? 'justify-center px-2 py-2' : 'gap-3 px-3 py-3'"
       :title="sidebarCollapsed ? {{ json_encode(explode(' ', auth()->user()->name)[0]) }} : ''">
        ...
        <div class="relative flex-shrink-0">
            ...
            @endif
            <div class="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-green-400 border-2 border-[#082B59]"></div>
        </div>
        ...
        <svg class="w-3.5 h-3.5 text-white/20 group-hover:text-white/50 flex-shrink-0 transition-all duration-200"
    ...
    {{-- ── Navegación ───────────────────────────────────────────── --}}
    <nav class="flex-1 overflow-y-auto overflow-x-hidden py-2 px-3 space-y-0.5 scrollbar-thin
        ...
        @foreach($navGroups as $group)
            ...
            {{-- Aula Virtual antes de "Sistema" --}}
            @if(isset($group['label']) && $group['label'] === 'Sistema' && $verGrupoAula)
                <div class="px-3 pt-4 pb-1 overflow-hidden transition-all duration-200" x-show="!sidebarCollapsed">
                    <p class="text-[10px] font-bold uppercase tracking-widest text-white/25 whitespace-nowrap">Aula Virtual</p>
                </div>
            ...
            @if($group['label'])
                <div class="px-3 pt-4 pb-1 overflow-hidden transition-all duration-200" x-show="!sidebarCollapsed">
                    <p class="text-[10px] font-bold uppercase tracking-widest text-white/25 whitespace-nowrap">
                        {{ $group['label'] }}
                    </p>
                </div>
            @elseif(!$loop->first)
            ...
            @foreach($itemsVisibles as $item)
                @php
                    ...
                    $iconBase  = $isActive ? 'bg-accent/20' : 'bg-white/5 group-hover:bg-white/10';
                    $iconColor = $isActive ? 'text-accent' : 'text-white/50 group-hover:text-white/80';
                @endphp

                @if($item['available'])
                    <a href="{{ route($item['route']) }}"
                       :title="sidebarCollapsed ? {{ json_encode($item['label']) }} : ''"
                       class="flex items-center py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group {{ $itemBase }}"
                       :class="sidebarCollapsed ? 'justify-center px-0 gap-0' : 'px-3 gap-3'">
                        <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-200 {{ $iconBase }}">
                            <svg class="{{ $iconColor }}" style="width:1.125rem;height:1.125rem"
                                 fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $item['icon'] }}"/>
                            </svg>
                        </div>
                        <span class="truncate transition-all duration-300 whitespace-nowrap overflow-hidden"
                              :class="sidebarCollapsed ? 'w-0 opacity-0' : 'flex-1 opacity-100'">
                            {{ $item['label'] }}
                        </span>
                        @if($isActive)
                            <span class="w-1.5 h-1.5 rounded-full bg-accent flex-shrink-0" x-show="!sidebarCollapsed"></span>
                        @endif
                    </a>
                @else
                    <div class="flex items-center py-2.5 rounded-xl text-sm font-medium
                                opacity-40 cursor-not-allowed select-none transition-all duration-200"
                         :class="sidebarCollapsed ? 'justify-center px-0 gap-0' : 'px-3 gap-3'"
                         :title="sidebarCollapsed ? {{ json_encode($item['label']) }} : ''">
                        ...
                        <span class="truncate text-white/40 transition-all duration-300 whitespace-nowrap overflow-hidden"
                              :class="sidebarCollapsed ? 'w-0 opacity-0' : 'flex-1 opacity-100'">
                            {{ $item['label'] }}
                        </span>
                        <span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-white/10 text-white/30 uppercase tracking-wide flex-shrink-0"
                              x-show="!sidebarCollapsed">
                            Pronto
                        </span>
                    </div>
        ...
        {{-- Aula Virtual para alumno/representante (sin grupo Sistema) --}}
        @if($verGrupoAula && !in_array($role, ['admin', 'docente']))
            @include('components.sidebar-aula-group')
        @endif

        {{-- ── Botón colapsar (solo desktop) ───────────────────── --}}
        <div class="mt-2 pt-2 border-t border-white/5 hidden md:block">
            <button @click="toggleCollapse()"
                    :title="sidebarCollapsed ? 'Expandir sidebar' : ''"
                    class="w-full flex items-center py-2 rounded-xl text-xs font-medium
    ...
    <div class="px-3 pb-4 pt-2 border-t border-white/10 flex-shrink-0">
        <form method="POST" action="{{ route('logout') }}">
            ...
            <button type="submit"
                    ...
                    class="w-full flex items-center py-2.5 rounded-xl text-sm font-medium
