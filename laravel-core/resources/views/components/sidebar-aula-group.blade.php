<div x-data="{ open: {{ $aulaVirtualActiva ? 'true' : 'false' }} }" class="space-y-0.5">
    ...
    <button type="button"
            ...
            :title="sidebarCollapsed ? 'Aula Virtual' : ''"
            class="w-full flex items-center py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group"
            :class="{
                'bg-accent/15 text-accent': open || {{ $aulaVirtualActiva ? 'true' : 'false' }},
                'text-white/60 hover:bg-white/10 hover:text-white': !(open || {{ $aulaVirtualActiva ? 'true' : 'false' }}),
                'justify-center px-0 gap-0': sidebarCollapsed,
                'px-3 gap-3': !sidebarCollapsed,
            }">

        <div :class="(open || {{ $aulaVirtualActiva ? 'true' : 'false' }}) ? 'bg-accent/20' : 'bg-white/5 group-hover:bg-white/10'"
             class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-200">
            <svg style="width:1.125rem;height:1.125rem" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                 :class="(open || {{ $aulaVirtualActiva ? 'true' : 'false' }}) ? 'text-accent' : 'text-white/50 group-hover:text-white/80'">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
            </svg>
        </div>

        <span class="flex-1 text-left truncate transition-all duration-300 whitespace-nowrap overflow-hidden"
              :class="sidebarCollapsed ? 'w-0 opacity-0' : 'opacity-100'">
            Aula Virtual
        </span>

        <svg class="w-3.5 h-3.5 flex-shrink-0 transition-all duration-200"
             :class="[
                 open ? 'rotate-180 text-accent' : 'text-white/20',
    ...
    {{-- Sub-items: solo visibles cuando expandido Y abierto --}}
